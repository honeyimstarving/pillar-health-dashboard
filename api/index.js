...
const CTM_ACCESS_KEY = process.env.CTM_ACCESS_KEY;  // Access Key from CTM API Integration page
const CTM_SECRET_KEY = process.env.CTM_SECRET_KEY;  // Secret Key from CTM API Integration page
...
async function fetchCTMCalls(dateFrom, dateTo, numbers) {
  ...
  while (true) {
    ...
    const res = await fetch(url, {
      headers: {
        'Authorization': 'Basic ' + Buffer.from(`${CTM_ACCESS_KEY}:${CTM_SECRET_KEY}`).toString('base64'),
